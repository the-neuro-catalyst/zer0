...
// Import new tools
import { analyzeProject, analyzeDependencies, getComponents, getRoutingStructure, getTailwindUsage, getHooksUsage, analyzeApiCalls, analyzeDatabaseSchema, getProjectResources, readProjectResource, getPromptTemplates, getPrompt, } from "./tools/index.js";
const mcpServer = new McpServer({
    name: "zero-mcp-gemini",
    version: "0.2.1",
});
...
const checkBinary = async (name) => {
    const path = join(await binaryPath, name);
    if (!existsSync(path)) {
        throw new Error(`Binary '${name}' not found at ${path}.\n` +
...
const execWithContext = async (binary, args, options = {}) => {
    const { maxBuffer = 1024 * 1024 * 50, // 50MB default
    timeout = DEFAULT_TIMEOUT, context = "operation", } = options;
    try {
        ...
        if (error.code === "ENOENT") {
            throw new Error(`Failed to execute '${binary}': command not found.\n` +
                `Make sure the binary is installed and in your PATH.`);
        }
        // Buffer overflow
        if (error.message.includes("maxBuffer")) {
            throw new Error(`${context} exceeded output buffer (${maxBuffer} bytes).\n` +
...
const projectPathArg = process.argv.find((arg) => arg.startsWith("--project-path="));
...
mcpServer.registerTool("inspect_resource", {
    description: "Deep analysis of a file or URL. Returns schema, metadata, and data samples. Supports Parquet, CSV, JSON, etc.",
...
mcpServer.registerTool("search_files", {
    description: "High-speed recursive search for a pattern within a directory using ripgrep. Supports regex.",
    inputSchema: z.object({
        pattern: z
            .string()
...
// Tool: List Tree
mcpServer.registerTool("list_tree", {
    description: "Provide a recursive tree view of the directory structure. Use this for a bird's-eye view of the project.",
    inputSchema: z.object({
...
mcpServer.registerTool("scan_patterns", {
    description: "Automated scan for sensitive patterns (PII, Secrets, API Keys) across the workspace. Can use preset patterns or custom ones.",
    inputSchema: z.object({
        dirPath: z
            .string()
            .optional()
        ...
        customPatterns: z
            .array(z.string())
            .optional()
            .describe("Custom regex patterns to search for (overrides presets if provided)"),
        excludeEmails: z
            .boolean()
            .optional()
            .default(false)
...
mcpServer.registerTool("query_database", {
    description: "Execute a read-only SQL query against a database (SQLite or PostgreSQL).",
    inputSchema: z.object({
        url: z
            .string()
            .describe('Database URL (e.g., "sqlite://data.db" or "postgres://user:pass@host/db")'),
...
mcpServer.registerTool("get_database_schema", {
    description: "Extract and visualize the schema (tables and columns) of a database.",
    inputSchema: z.object({
...
mcpServer.registerTool("analyze_join_keys", {
    description: "Analyze schemas of two data sources (File vs Database or File vs File) to suggest potential join keys.",
    inputSchema: z.object({
        sourceA: z
            .string()
    ...
    try {
        ...
        const getSchema = async (src) => {
            ...
            try {
                ...
                throw new Error(`Failed to parse schema JSON from ${src}. Output: ${stdout.substring(0, 200)}...`);
            }
        };
        // Extract columns with robust fallbacks
        const getColumns = (data, source) => {
            try {
                ...
                for (const format of possibleFormats) {
                    if (data[format] && Array.isArray(data[format])) {
                        ...
                        if (firstRecord?.column_schemas) {
                            return firstRecord.column_schemas.map((c) => c.name);
                        }
                    }
                }
                ...
                if (firstKey && Array.isArray(data[firstKey])) {
                    ...
                    if (item?.column_schemas) {
                        return item.column_schemas.map((c) => c.name);
                    }
                }
                throw new Error(`Unrecognized schema format from ${source}. Expected Parquet/CSV/JSON file or database schema.`);
            }
            catch (e) {
                throw new Error(`Failed to extract columns from ${source}: ${e.message}`);
            }
        };
        ...
        const potentialKeys = colsA.filter((a) => colsB.some((b) => a.toLowerCase() === b.toLowerCase()));
        const result = {
            sourceA_columns: colsA,
            sourceB_columns: colsB,
            potential_join_keys: potentialKeys.length > 0
                ? potentialKeys
            ...
            analysis: {
                ...
                recommendation: potentialKeys.length > 0
...
// Tool: Ingest Data
mcpServer.registerTool("ingest_data", {
    description: "Ingest data into a SQLite database with optional vector embeddings.",
    inputSchema: z.object({
...
}, async ({ db_path, collection_name, vector_size, openai_api_key, embed_field, path, }) => {
    try {
...
mcpServer.registerTool("analyze_project", {
...
mcpServer.registerTool("analyze_dependencies", {
...
// Tool: Get Components
mcpServer.registerTool("get_components", {
...
mcpServer.registerTool("get_routing_structure", {
...
mcpServer.registerTool("get_tailwind_usage", {
...
mcpServer.registerTool("get_hooks_usage", {
...
mcpServer.registerTool("analyze_api_calls", {
...
mcpServer.registerTool("analyze_database_schema", {
    description: "Analyze database schema, tables, types, functions, policies, and relationships",
    inputSchema: z.object({}).shape,
}, async () => {
    try {
        return {
...
await (async () => {
    try {
        ...
        for (const template of promptTemplates) {
            ...
            mcpServer.registerPrompt(template.name, {
                title: template.name
                    .replace(/_/g, " ")
...
async function main() {
    try {
        ...
        console.error("Server is ready with enhanced analysis tools, resources, and prompts...");
    }
    catch (error) {
