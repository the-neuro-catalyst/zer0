...
export async function getComponents(resolvedPath) {
    try {
...
export async function getRoutingStructure(resolvedPath) {
    try {
...
export async function getTailwindUsage(resolvedPath) {
    try {
        ...
        const patterns = {
            layout: sortedClasses.filter((item) => /^(flex|grid|block|inline|hidden|w-|h-|p-|m-|space-|gap-)/.test(item.class)),
            colors: sortedClasses.filter((item) => /^(bg-|text-|border-|from-|to-|via-)/.test(item.class)),
            typography: sortedClasses.filter((item) => /^(text-|font-|leading-|tracking-|uppercase|lowercase)/.test(item.class)),
            responsive: sortedClasses.filter((item) => /^(sm:|md:|lg:|xl:|2xl:)/.test(item.class)),
        };
        const result = {
...
export async function getHooksUsage(resolvedPath) {
    try {
