...
import {
  ApiCallDetail,
  ApiCallsAnalysisResult,
  SchemaTableDetail,
  SchemaTypeDetail,
  SchemaFunctionDetail,
  SchemaPolicyDetail,
  SchemaRelationshipDetail,
  SupabaseUsageDetail,
  DatabaseSchema,
  DatabaseSchemaAnalysisResult,
} from "../types/index.js"; // Import new types

export async function analyzeApiCalls(resolvedPath: string): Promise<string> {
  try {
    ...
    // Analyze patterns
    const apiTypes: { [key: string]: number } = {};
    ...
    const result: ApiCallsAnalysisResult = {
...
export async function analyzeDatabaseSchema(
  ...
  try {
    ...
    const supabaseTypes: SupabaseUsageDetail[] = [];
    ...
    for (const file of schemaFiles) {
      ...
      const tableMatches =
        content.match(/CREATE\s+TABLE\s+(\w+)\s*\([^)]+\)/gi) || [];
      ...
      const interfaceMatches =
        content.match(
          /interface\s+(\w*(?:Database|Table|Row|Insert|Update)\w*)\s*\{[^}]+\}/gi,
        ) || [];
      ...
      const typeMatches =
        content.match(
          /type\s+(\w*(?:Database|Table|Row|Insert|Update)\w*)\s*=[^;]+/gi,
        ) || [];
      ...
      // RLS Policies
      const policyMatches =
        content.match(/CREATE\s+POLICY\s+(\w+)\s+ON\s+(\w+)/gi) || [];
      policyMatches.forEach((match) => {
        const policyMatch = match.match(
      ...
      const functionMatches =
        content.match(/CREATE\s+(?:OR\s+REPLACE\s+)?FUNCTION\s+(\w+)/gi) || [];
      ...
      const foreignKeyMatches =
        content.match(/REFERENCES\s+(\w+)\s*\(/gi) || [];
      ...
      if (content.includes("supabase") || content.includes("Database")) {
        ...
        const supabaseTableRefs =
          content.match(/from\s*\(\s*["'](\w+)["']\s*\)/gi) || [];
    ...
    const supabaseUsage: SupabaseUsageDetail[] = [];
    ...
    const result: DatabaseSchemaAnalysisResult = {
      schemaFiles,
