...
pub fn read_yaml_value(path: &Path, _head: Option<usize>) -> Result<YamlData, DataReaderError> {
...
pub fn get_yaml_raw_content(path: &Path, _head: Option<usize>) -> Result<String, DataReaderError> {
    let mut file = File::open(path)
        .map_err(|e| DataReaderError::FileReadError { path: path.to_path_buf(), source: e })?;
